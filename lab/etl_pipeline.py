...
def cmd_embed_internal(cleaned_csv: Path, *, run_id: str, log) -> bool:
    ...
    try:
        ...
        # Fallback offline: embedding hash cục bộ, không cần tải model từ mạng.
        emb = None
        using_hash_embeddings = True
        log(f"WARN: sentence-transformer unavailable, fallback local_hash_embeddings :: {e}")

    if using_hash_embeddings:
        try:
            client.delete_collection(name=collection_name)
            log("embed_collection_reset=1 (offline hash fallback)")
        except Exception:
            pass
    ...
    if using_hash_embeddings:
        ...
        vectors = [hash_embed(doc) for doc in documents]
