...
> **MỘT LỆNH CHẠY CẢ PIPELINE (END-TO-END DOD):**
> Nhóm Antigravity đã cài đặt tích hợp toàn bộ các Stage: Ingest -> Clean -> Validate -> Embed Idempotent chỉ qua duy nhất một Command Line.
```bash
# Luồng chuẩn: fix stale refund 14→7, expectation pass, embed
python etl_pipeline.py run
```

# Kiểm tra freshness riêng biệt báo cáo SLA
```bash
